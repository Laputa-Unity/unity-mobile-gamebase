...
namespace custom.find.reference
{
	...
	public class CustomWindowAll : CustomWindowBase, IHasCustomMenu
	{
		...
		[NonSerialized] internal CustomBookmark bookmark;
		[NonSerialized] internal CustomSelection selection;
		

		[NonSerialized] internal CustomUsedInBuild UsedInBuild;
		[NonSerialized] internal CustomDuplicateTree2 Duplicated;
		[NonSerialized] internal CustomRefDrawer RefUnUse;
		
		[NonSerialized] internal CustomRefDrawer UsesDrawer;			// [Selected Assets] are [USING] (depends on / contains reference to) ---> those assets
		[NonSerialized] internal CustomRefDrawer UsedByDrawer;		// [Selected Assets] are [USED BY] <---- those assets 
		[NonSerialized] internal CustomRefDrawer SceneToAssetDrawer;	// [Selected GameObjects in current Scene] are [USING] ---> those assets

		[NonSerialized] internal CustomRefDrawer RefInScene;			// [Selected Assets] are [USED BY] <---- those components in current Scene 
		[NonSerialized] internal CustomRefDrawer SceneUsesDrawer;		// [Selected GameObjects] are [USING] ---> those components / GameObjects in current scene
		[NonSerialized] internal CustomRefDrawer RefSceneInScene;		// [Selected GameObjects] are [USED BY] <---- those components / GameObjects in current scene
		...
		protected void InitIfNeeded()
		{
			...
			CustomCache.onReady -= OnReady;
			CustomCache.onReady += OnReady;

            CustomSetting.OnIgnoreChange -= OnIgnoreChanged;
            CustomSetting.OnIgnoreChange += OnIgnoreChanged;
		...
		public override void OnSelectionChange()
		{
			...
			if (!lockSelection)
			{
				ids = CustomUnity.Selection_AssetGUIDs;
				selection.Clear();
			
				//ignore selection on asset when selected any object in scene
				if (Selection.gameObjects.Length > 0 && !CustomUnity.IsInAsset(Selection.gameObjects[0]))
				{
					ids = new string[0];
			...
			if (CustomSceneCache.Api.Dirty && !Application.isPlaying)
			{
				CustomSceneCache.Api.refreshCache(this);
			}
		...
		public CustomSplitView sp1;	// container : Selection / sp2 / Bookmark 
		public CustomSplitView sp2;	// Scene / Assets
		
		void InitPanes()
		{
			sp2 = new CustomSplitView(this)
			{
				isHorz = false,
				splits = new List<CustomSplitView.Info>()
				{
					new CustomSplitView.Info(){ title = new GUIContent("Scene", CustomIcon.Scene.image), draw = DrawScene },
					new CustomSplitView.Info(){ title = new GUIContent("Assets", CustomIcon.Asset.image), draw = DrawAsset },
				}
			};

			sp2.CalculateWeight();
			
			sp1 = new CustomSplitView(this)
			{
				isHorz = true,
				splits = new List<CustomSplitView.Info>()
				{
					new CustomSplitView.Info(){ title = new GUIContent("Selection", CustomIcon.Selection.image), weight = 0.4f, visible = false, draw = (rect) => selection.Draw(rect) },
					new CustomSplitView.Info(){ draw = (r) =>
					{
					...
					new CustomSplitView.Info(){ title = new GUIContent("Bookmark", CustomIcon.Favorite.image), weight = 0.4f, visible = false, draw = (rect) => bookmark.Draw(rect) },
				}
			};
		...
		private CustomTabView bottomTabs;
		private CustomSearchView search;
		
		void DrawScene(Rect rect)
		{
			CustomRefDrawer drawer = IsFocusingUses
			...
			if (!CustomSceneCache.ready)
			{
				...
				int cur = CustomSceneCache.Api.current, total = CustomSceneCache.Api.total;
			...
			if (GUI.ColorIconButton(refreshRect, CustomIcon.Refresh.image,
				CustomSceneCache.Api.Dirty ? (Color?)GUI.lightRed : null))
			{
				CustomSceneCache.Api.refreshCache(drawer.window);
			}
		}
		
		

		CustomRefDrawer GetAssetDrawer()
		{
			if (IsFocusingUses)
			{
		...
		void DrawSearch()
		{
			if (search == null) search = new CustomSearchView();
			search.DrawLayout();
		}
		...
		protected bool CheckDrawImport()
		{
			...
			{
				EditorGUILayout.HelpBox("CR requires serialization mode set to FORCE TEXT!", MessageType.Warning);
			...
			if (CustomCache.hasCache && !CustomCache.CheckSameVersion())
			{
				EditorGUILayout.HelpBox("Incompatible cache version found!!!\nCR will need a full refresh and this may take quite some time to finish but you would be able to work normally while the scan works in background!",
					MessageType.Warning);
				CustomCache.DrawPriorityGUI();
				if (GUILayout.Button("Scan project"))
				{
					CustomCache.DeleteCache();
		...
		void InitTabs()
		{
			tabs = CustomTabView.Create(this, false, 
			...
			tabs.callback = new DrawCallback()
			{
				BeforeDraw = ()=>
				{
					if (GUI.ToolbarToggle(ref selection.isLock, 
						selection.isLock ? CustomIcon.Lock.image : CustomIcon.Unlock.image,
				...
				AfterDraw = () =>
				{
					...
					if (GUI.ToolbarToggle(ref sp1.isHorz, CustomIcon.Panel.image, Vector2.zero, "Layout"))
					{
						sp1.CalculateWeight();
						Repaint();
					}
					
					if (GUI.ToolbarToggle(ref sp1.splits[0].visible, CustomIcon.Selection.image, Vector2.zero, "Show / Hide Selection"))
					{
						sp1.CalculateWeight();
						Repaint();
					}
					
					if (GUI.ToolbarToggle(ref sp2.splits[0].visible, CustomIcon.Scene.image, Vector2.zero, "Show / Hide Scene References"))
					{
						sp2.CalculateWeight();
						Repaint();
					}
					
					if (GUI.ToolbarToggle(ref sp2.splits[1].visible, CustomIcon.Asset.image, Vector2.zero, "Show / Hide Asset References"))
					{
						sp2.CalculateWeight();
						Repaint();
					}
					...
					if (GUI.ToolbarToggle(ref sp1.splits[2].visible, CustomIcon.Favorite.image, Vector2.zero, "Show / Hide Bookmarks"))
					{
						sp1.CalculateWeight();
						Repaint();
					}
		...
		protected bool DrawHeader()
		{
			...
			if (bottomTabs == null)
			{
				bottomTabs = CustomTabView.Create(this, true, 
					new GUIContent(CustomIcon.Setting.image, "Settings"),
                    new GUIContent(CustomIcon.Ignore.image, "Ignore"),
                    new GUIContent(CustomIcon.Filter.image, "Filter by Type")
				);
		...
		void DrawAssetViewSettings()
		{
			...
			{
				GUI.ToolbarToggle(ref CustomSetting.s.displayAssetBundleName, CustomIcon.AssetBundle.image, Vector2.zero, "Show / Hide Assetbundle Names");
				#if UNITY_2017_1_OR_NEWER
				GUI.ToolbarToggle(ref CustomSetting.s.displayAtlasName, CustomIcon.Atlas.image, Vector2.zero, "Show / Hide Atlas packing tags");
				#endif
				GUI.ToolbarToggle(ref CustomSetting.s.showUsedByClassed, CustomIcon.Material.image, Vector2.zero, "Show / Hide usage icons");
				GUI.ToolbarToggle(ref CustomSetting.s.displayFileSize, CustomIcon.Filesize.image, Vector2.zero, "Show / Hide file size");
		...
		void DrawViewModes()
		{
			var gMode = CustomSetting.GroupMode;
			if (GUI.EnumPopup(ref gMode, new GUIContent(CustomIcon.Group.image, "Group by"), EditorStyles.toolbarPopup, GUILayout.Width(80f)))
			{
				CustomSetting.GroupMode = gMode;
				markDirty();
			}
					
			GUILayout.Space(16f);
					
			var sMode = CustomSetting.SortMode;
			if (GUI.EnumPopup(ref sMode, new GUIContent(CustomIcon.Sort.image, "Sort by"), EditorStyles.toolbarPopup, GUILayout.Width(50f)))
			{
				CustomSetting.SortMode = sMode;
				RefreshSort();
			}
		}
		...
		private CustomDeleteButton deleteUnused;
			...
			if (IsFocusingUnused)
			{
				...
				if ((RefUnUse.refs != null && RefUnUse.refs.Count == 0))
				{
					GUILayout.BeginArea(rect);
					{
						...
						EditorGUILayout.HelpBox("Your backups are placed at Library/CR/ just in case you want your assets back!", MessageType.Info);	
					}
					GUILayout.EndArea();
				}
				else
				{
					...
					GUI.Rect(lineRect, Color.black, 0.5f);
					...
					{
						deleteUnused = new CustomDeleteButton()
						{
							...
							deleteLabel = new GUIContent("DELETE ASSETS", CustomIcon.Delete.image),
							confirmMessage = "Create backup at Library/CR/"
						};
					}
				
					GUILayout.BeginArea(toolRect);
					deleteUnused.Draw(()=> { CustomUnity.BackupAndDeleteAssets(RefUnUse.source); });
					GUILayout.EndArea();
				}
				return;
			}
		...
		void DrawSettings()
		{
			...
			rect.height = 1f;
			GUI.Rect(rect, Color.black, 0.4f);
		}
		...
		private void DrawGUIDs()
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				if (guid != tempGUID && !string.IsNullOrEmpty(guid))
				{
					...
					tempObject = CustomUnity.LoadAssetAtPath<Object>
					(
			...
			{
				CustomExport.MergeDuplicate(tempGUID);
			}
