...
{
	public class CustomSelection : IRefDraw
	{	
		...
		public void Remove(CustomRef rf)
		{
			if (rf.isSceneRef)
			{
				Remove(rf.component);
		...
		public CustomSelection(IWindow window)
		{
			this.window = window;
		...
		public void DrawLock(Rect rect)
		{
			GUI.ContentColor(() =>
			{
				var icon = isLock ? CustomIcon.Lock : CustomIcon.Unlock;
				if (GUI.Toggle(rect, ref isLock, icon))
				{
