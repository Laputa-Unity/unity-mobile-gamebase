...
namespace custom.find.reference
{
 ...
 typeof(Selection).GetProperty("assetGUIDs", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
		...
		internal static void RepaintCRWindows()
		{
			RepaintAllEditor("vietlabs.fr2.CustomWindow");
		}
        ...
        public static void BackupAndDeleteAssets(CustomRef[] assets)
        {
            ...
            Directory.CreateDirectory("Library/CR/");
            AssetDatabase.ExportPackage(result.ToArray(), "Library/CR/bk_" + fileName + ".unitypackage");
