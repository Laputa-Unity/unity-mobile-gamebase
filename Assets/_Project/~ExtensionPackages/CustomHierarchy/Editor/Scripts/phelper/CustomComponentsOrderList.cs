...
{
    public class CustomComponentsOrderList
    {
        // PRIVATE
        ...
        // CONSTRUCTOR
        public CustomComponentsOrderList (EditorWindow window)
        {            
            this.window = window;
            dragButton = CustomResources.getInstance().GetTexture(CustomTexture.CustomDragButton);
            backgroundColor = CustomResources.getInstance().GetColor(CustomColor.BackgroundDark);
        }
        ...
        public void draw(Rect rect, string[] componentIds)
        {
            ...
            if (dragAndDrop && currentEvent.type == EventType.MouseUp)      
            {
                ...
                if (currentMouseIndex != originalDragIndex)
                {
                    ...
                    CustomSettings.getInstance().set(CustomSetting.ComponentsOrder, newIconOrder);
            ...
            {
                CustomHierarchyComponentEnum type = (CustomHierarchyComponentEnum)int.Parse(componentIds[i]);
            ...
            if (dragAndDrop)
            {
                ...
                drawComponentLabel(new Rect(rect.x, curY, rect.width, rect.height), (CustomHierarchyComponentEnum)int.Parse(componentIds[originalDragIndex]), true);
            }
        }
        
        // PRIVATE
        private void drawComponentLabel(Rect rect, CustomHierarchyComponentEnum type, bool withBackground = false)
        {
            if (withBackground)
            {
