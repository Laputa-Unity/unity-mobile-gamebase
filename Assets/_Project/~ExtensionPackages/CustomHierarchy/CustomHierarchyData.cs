...
{
    public class CustomHierarchyData : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public void OnBeforeSerialize() => CustomHierarchy.OnDataSerialization();
                ...
                get
                {
                    if (_go == null)
                        if (!failedToLoadSceneObject) // to prevent continuous GlobalID.GetObjects() calls if object is deleted
                            CustomHierarchy.unloadedSceneBookmarks_sceneGuids.Add(globalId.guid);
        ...
        class Editor : UnityEditor.Editor
        {
            public override void OnInspectorGUI()
            {
                ...
                void teamMode()
                {
                    ...
                    Space(10);
                    EditorGUILayout.LabelField("Now that Team Mode is enabled, create an empty script that inherits from CustomHierarchy.CustomHierarchyDataComponent and add it to any object in a scene.", style);
        ...
        public static bool teamModeEnabled { get => EditorPrefsCached.GetBool("CustomHierarchy-teamModeEnabled", false); set => EditorPrefsCached.SetBool("CustomHierarchy-teamModeEnabled", value); }
        ...
        public void EnableTeamMode()
        {
            var option = EditorUtility.DisplayDialogComplex("Licensing notice",
                                                            "To use CustomHierarchy 2 within a team, licenses must be purchased for each individual user as per the Asset Store EULA.\n\n Sharing one license across the team is illegal and considered piracy.",
                                                            "Acknowledge",
                                                            "Cancel",
                                                            "Purchase more seats");
            if (option == 2)
                Application.OpenURL("https://prf.hn/click/camref:1100lGLBn/pubref:teammode/destination:https://assetstore.unity.com/packages/tools/utilities/CustomHierarchy-2-253397");
            ...
            CustomHierarchy.goDataCache.Clear();
