...
using CustomBuildReport;
using CustomBuildReport.Window;
...
namespace CustomBuildReport
{
	public class AssetBundleSession
	{
		...
		public CustomBuildReport.SizePart[] TopLargest;
		...
		public void Refresh(BuildInfo buildInfo)
		{
			...
			{
				var topLargest = new List<CustomBuildReport.SizePart>();
				int numberOfTop = CustomBuildReport.Options.NumberOfTopLargestUsedAssetsToShow;
...
public class BRT_BuildReportWindow : EditorWindow
{
	...
	void OnEnable()
	{
		...
		if (CustomBuildReport.Util.BuildInfoHasContents(_buildInfo))
		{
			//Debug.Log("recompiled " + _buildInfo.SavedPath);
			if (!string.IsNullOrEmpty(_buildInfo.SavedPath))
			{
				CustomBuildReport.BuildInfo loadedBuild = CustomBuildReport.Util.OpenSerializedBuildInfo(_buildInfo.SavedPath);
				if (CustomBuildReport.Util.BuildInfoHasContents(loadedBuild))
				{
			...
			else
			{
				if (_buildInfo.HasUsedAssets)
				{
					_buildInfo.UsedAssets.AssignPerCategoryList(
						CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(_buildInfo.UsedAssets.All,
				...
				if (_buildInfo.HasUnusedAssets)
				{
					_buildInfo.UnusedAssets.AssignPerCategoryList(
						CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(_buildInfo.UnusedAssets.All,
							_buildInfo.FileFilters));
				}
			}
		}

		_usedAssetsScreen.SetListToDisplay(CustomBuildReport.Window.Screen.AssetList.ListToDisplay.UsedAssets);
		_unusedAssetsScreen.SetListToDisplay(CustomBuildReport.Window.Screen.AssetList.ListToDisplay.UnusedAssets);
	...
	void OnInspectorUpdate()
	{
		...
		if (_buildInfo != null && CustomBuildReport.ReportGenerator.IsFinishedGettingValues)
		{
			OnFinishGeneratingBuildReport();
		}

		// if Unity Editor has finished making a build and we are scheduled to create a Build Report...
		if (!CustomBuildReport.ReportGenerator.IsStillGettingValues &&
		    !EditorApplication.isCompiling &&
		    CustomBuildReport.Util.ShouldGetBuildReportNow)
		{
	...
	readonly CustomBuildReport.Window.Screen.Overview _overviewScreen = new CustomBuildReport.Window.Screen.Overview();

	readonly CustomBuildReport.Window.Screen.BuildSettings _buildSettingsScreen =
		new CustomBuildReport.Window.Screen.BuildSettings();

	readonly CustomBuildReport.Window.Screen.BuildStepsScreen _buildStepsScreen =
		new CustomBuildReport.Window.Screen.BuildStepsScreen();

	readonly CustomBuildReport.Window.Screen.SizeStats _sizeStatsScreen = new CustomBuildReport.Window.Screen.SizeStats();
	readonly CustomBuildReport.Window.Screen.AssetList _usedAssetsScreen = new CustomBuildReport.Window.Screen.AssetList();
	readonly CustomBuildReport.Window.Screen.AssetList _unusedAssetsScreen = new CustomBuildReport.Window.Screen.AssetList();
	readonly CustomBuildReport.Window.Screen.ExtraData _extraDataScreen = new CustomBuildReport.Window.Screen.ExtraData();

	readonly CustomBuildReport.Window.Screen.Options _optionsScreen = new CustomBuildReport.Window.Screen.Options();
	readonly CustomBuildReport.Window.Screen.Help _helpScreen = new CustomBuildReport.Window.Screen.Help();
	...
	/// </summary>
	static CustomBuildReport.BuildInfo _buildInfo;

	/// <summary>
	/// The Asset Dependencies data being used
	/// for whichever Build Report is displayed.
	/// </summary>
	static CustomBuildReport.AssetDependencies _assetDependencies;

	/// <summary>
	/// The TextureData being used
	/// for whichever Build Report is displayed.
	/// </summary>
	static CustomBuildReport.TextureData _textureData;

	/// <summary>
	/// The MeshData being used
	/// for whichever Build Report is displayed.
	/// </summary>
	static CustomBuildReport.MeshData _meshData;

	/// <summary>
	/// The PrefabData being used
	/// for whichever Build Report is displayed.
	/// </summary>
	static CustomBuildReport.PrefabData _prefabData;

	static CustomBuildReport.UnityBuildReport _unityBuildReport;
		...
		BuildInfo buildReportToDisplay, AssetDependencies assetDependencies)
	{
		...
		if (CustomBuildReport.Options.ShowAssetPrimaryUsersInTooltipIfAvailable && !alreadyUsingSameAssetPath)
		{
	...
	{
		CustomBuildReport.AssetDependencies.PopulateAssetEndUsers(HoveredAssetEntryPath, assetDependencies);
	...
	void RecategorizeDisplayedBuildInfo()
	{
		if (CustomBuildReport.Util.BuildInfoHasContents(_buildInfo))
		{
			CustomBuildReport.ReportGenerator.RecategorizeAssetList(_buildInfo);
		}
	}
	...
	void InitGUISkin()
	{
		...
		{
			guiSkinToUse = CustomBuildReport.Window.Settings.DARK_GUI_SKIN_FILENAME;
		}
		else
		{
			guiSkinToUse = CustomBuildReport.Window.Settings.DEFAULT_GUI_SKIN_FILENAME;
		}

		// try default path
		_usedSkin = AssetDatabase.LoadAssetAtPath(
			            string.Format("{0}/GUI/{1}", CustomBuildReport.Options.BUILD_REPORT_TOOL_DEFAULT_PATH, guiSkinToUse),
	...
	public void Init(CustomBuildReport.BuildInfo buildInfo)
	{
	...
	{
		GoToOverviewScreen();
		CustomBuildReport.ReportGenerator.RefreshData(fromBuild, _assetBundleSession, ref _buildInfo, ref _assetDependencies, ref _textureData, ref _meshData, ref _prefabData);
	}

	bool IsWaitingForBuildCompletionToGenerateBuildReport
	{
		get { return EditorApplication.isCompiling && CustomBuildReport.Util.ShouldGetBuildReportNow; }
	}
	...
	{
		CustomBuildReport.ReportGenerator.OnFinishedGetValues(_buildInfo, _assetDependencies, _textureData, _meshData, _prefabData);
	...
	void _OpenBuildInfo(string filepath)
	{
		...
		CustomBuildReport.BuildInfo loadedBuild = CustomBuildReport.Util.OpenSerializedBuildInfo(filepath, false);
		...
		var assetDependenciesFilePath = CustomBuildReport.Util.GetAssetDependenciesFilenameFromBuildInfo(filepath);
		if (System.IO.File.Exists(assetDependenciesFilePath))
		{
			var loadedAssetDependencies = CustomBuildReport.Util.OpenSerialized<CustomBuildReport.AssetDependencies>(assetDependenciesFilePath);
		...
		var textureDataFilePath = CustomBuildReport.Util.GetTextureDataFilenameFromBuildInfo(filepath);
		if (System.IO.File.Exists(textureDataFilePath))
		{
			var loadedTextureData = CustomBuildReport.Util.OpenSerialized<CustomBuildReport.TextureData>(textureDataFilePath);
		...
		var meshDataFilePath = CustomBuildReport.Util.GetMeshDataFilenameFromBuildInfo(filepath);
		if (System.IO.File.Exists(meshDataFilePath))
		{
			var loadedMeshData = CustomBuildReport.Util.OpenSerialized<CustomBuildReport.MeshData>(meshDataFilePath);
		...
		var prefabDataFilePath = CustomBuildReport.Util.GetPrefabDataFilenameFromBuildInfo(filepath);
		if (System.IO.File.Exists(prefabDataFilePath))
		{
			var loadedPrefabData = CustomBuildReport.Util.OpenSerialized<CustomBuildReport.PrefabData>(prefabDataFilePath);
		...
		var unityBuildReportFilePath = CustomBuildReport.Util.GetUnityBuildReportFilenameFromBuildInfo(filepath);
		if (System.IO.File.Exists(unityBuildReportFilePath))
		{
			try
			{
				var loadedUnityBuildReport =
					CustomBuildReport.Util.OpenSerialized<CustomBuildReport.UnityBuildReport>(unityBuildReportFilePath);
		...
		var extraDataFilePath = CustomBuildReport.Util.GetExtraDataFilename(filepath).Replace('\\', '/');
	...
	void OpenBuildInfoInBackgroundIfNeeded(string filepath)
	{
		...
		if (!CustomBuildReport.Options.UseThreadedFileLoading)
		{
	...
	void DrawTopRowButtons()
	{
		...
		var leftToolbarStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TOOLBAR_LEFT_STYLE_NAME);
		if (leftToolbarStyle == null)
		{
			leftToolbarStyle = GUI.skin.button;
		}

		var midToolbarStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TOOLBAR_MIDDLE_STYLE_NAME);
		if (midToolbarStyle == null)
		{
			midToolbarStyle = GUI.skin.button;
		}

		var rightToolbarStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TOOLBAR_RIGHT_STYLE_NAME);
		    ...
		    !LoadingValuesFromThread)
		{
			string filepath = EditorUtility.OpenFilePanel(
				...
				CustomBuildReport.Options.BuildReportSavePath,
				"xml");
		...
		if (GUI.Button(new Rect(toolbarX, 5, 40, 40), _toolbarLabelSave, rightToolbarStyle) &&
		    CustomBuildReport.Util.BuildInfoHasContents(_buildInfo))
		{
			...
			if (!string.IsNullOrEmpty(filepath))
			{
				CustomBuildReport.Util.Serialize(_buildInfo, filepath);

				if (_assetDependencies != null && _assetDependencies.HasContents)
				{
					var assetDependenciesFilePath = CustomBuildReport.Util.GetAssetDependenciesFilenameFromBuildInfo(filepath);
					CustomBuildReport.Util.Serialize(_assetDependencies, assetDependenciesFilePath);
				}

				if (_textureData != null && _textureData.HasContents)
				{
					var textureDataFilePath = CustomBuildReport.Util.GetTextureDataFilenameFromBuildInfo(filepath);
					CustomBuildReport.Util.Serialize(_textureData, textureDataFilePath);
				}

				if (_meshData != null && _meshData.HasContents)
				{
					var meshDataFilePath = CustomBuildReport.Util.GetMeshDataFilenameFromBuildInfo(filepath);
					CustomBuildReport.Util.Serialize(_meshData, meshDataFilePath);
				}

				if (_prefabData != null && _prefabData.HasContents)
				{
					var prefabDataFilePath = CustomBuildReport.Util.GetPrefabDataFilenameFromBuildInfo(filepath);
					CustomBuildReport.Util.Serialize(_prefabData, prefabDataFilePath);
				}

				if (_unityBuildReport != null)
				{
					var unityBuildReportFilePath = CustomBuildReport.Util.GetUnityBuildReportFilenameFromBuildInfo(filepath);
					CustomBuildReport.Util.Serialize(_unityBuildReport, unityBuildReportFilePath);
				}
			}
		}
		...
		if (GUI.Button(new Rect(toolbarX, 5, 55, 40), _toolbarLabelOptions, leftToolbarStyle))
		{
			...
			CustomBuildReport.Options.UpdatePreviousSearchType();
		}
	...
	void OnGUI()
	{
		if (Event.current.type == EventType.Layout)
		{
			...
			_buildInfoHasNoContentsToDisplay = !CustomBuildReport.Util.BuildInfoHasContents(_buildInfo);
		}

		//GUI.Label(new Rect(5, 100, 800, 20), "CustomBuildReport.Util.ShouldReload: " + CustomBuildReport.Util.ShouldReload + " EditorApplication.isCompiling: " + EditorApplication.isCompiling);
		if (!_noGuiSkinFound)
		{
			GUI.skin = _usedSkin;
			//GUI.Label(new Rect(20, 20, 500, 100), CustomBuildReport.Options.BUILD_REPORT_PACKAGE_MISSING_MSG);
			//return;
		}
		else
		{
			GUI.Label(new Rect(300, -25, 500, 100), CustomBuildReport.Options.BUILD_REPORT_GUI_SKIN_MISSING_MSG);
		}

		DrawTopRowButtons();

		if (GUI.skin.FindStyle(CustomBuildReport.Window.Settings.VERSION_STYLE_NAME) != null)
		{
			GUI.Label(new Rect(0, 0, position.width, 20), CustomBuildReport.Info.ReadableVersion,
				CustomBuildReport.Window.Settings.VERSION_STYLE_NAME);
		}
		else
		{
			GUI.Label(new Rect(position.width - 160, 0, position.width, 20), CustomBuildReport.Info.ReadableVersion);
		}
		...
		var leftTabStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TAB_LEFT_STYLE_NAME);
		if (leftTabStyle == null)
		{
			leftTabStyle = GUI.skin.button;
		}

		var midTabStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TAB_MIDDLE_STYLE_NAME);
		if (midTabStyle == null)
		{
			midTabStyle = GUI.skin.button;
		}

		var rightTabStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TAB_RIGHT_STYLE_NAME);
		...
		{
			if (_selectedCategoryIdx != USED_ASSETS_IDX && CustomBuildReport.Options.HasSearchTypeChanged)
			{
		...
		{
			if (_selectedCategoryIdx != UNUSED_ASSETS_IDX && CustomBuildReport.Options.HasSearchTypeChanged)
			{
		...
		if (oldSelectedCategoryIdx != OPTIONS_IDX && _selectedCategoryIdx == OPTIONS_IDX)
		{
			...
			_fileFilterGroupToUseOnOpeningOptionsWindow = CustomBuildReport.Options.FilterToUseInt;
		}
		else if (oldSelectedCategoryIdx == OPTIONS_IDX && _selectedCategoryIdx != OPTIONS_IDX)
		{
			// moving away from the options screen
			_fileFilterGroupToUseOnClosingOptionsWindow = CustomBuildReport.Options.FilterToUseInt;
	...
	public static Texture GetAssetPreview(string assetName)
	{
		...
		else //if (_assetListEntryHovered.Name.EndsWith(".prefab") || CustomBuildReport.Util.IsFileAUnityMesh(_assetListEntryHovered.Name))
		{
#if UNITY_5_6_OR_NEWER
	...
	public static Vector2 GetThumbnailSize()
	{
		...
		thumbnailSize.x = ZoomedInThumbnails
			                  ? CustomBuildReport.Options.TooltipThumbnailZoomedInWidth
			                  : CustomBuildReport.Options.TooltipThumbnailWidth;

		thumbnailSize.y = ZoomedInThumbnails
			                  ? CustomBuildReport.Options.TooltipThumbnailZoomedInHeight
			                  : CustomBuildReport.Options.TooltipThumbnailHeight;
	...
	public static void DrawThumbnailTooltip(Rect position, CustomBuildReport.TextureData textureData)
	{
	...
	static bool GetTextureDataForTooltip(string assetPath, CustomBuildReport.TextureData textureData, out Vector2 labelSize)
	{
		if (textureData == null)
		{
	...
	public static void DrawThumbnailTooltip(Rect position, string assetPath, Rect assetRect, CustomBuildReport.TextureData textureData)
	{
		var thumbnailImage = BRT_BuildReportWindow.GetAssetPreview(assetPath);
	...
	public static void DrawThumbnailEndUsersTooltip(Rect position, AssetDependencies assetDependencies, CustomBuildReport.TextureData textureData)
	{
		List<GUIContent> endUsersListToUse = GetEndUserLabelsFor(assetDependencies, HoveredAssetEntryPath);
	...
	public static void DrawThumbnailEndUsersTooltip(Rect position, string assetPath, GUIContent label,
		List<GUIContent> endUsers, Rect assetRect, CustomBuildReport.TextureData textureData)
	{
		var thumbnailImage = BRT_BuildReportWindow.GetAssetPreview(assetPath);
