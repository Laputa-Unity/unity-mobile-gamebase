...
namespace CustomBuildReport.Window.Screen
{
	public partial class AssetList
	{
		...
		/// <summary>
		/// "which is explicitly added into the Asset Bundle"
		/// </summary>
		static readonly GUIContent AssetUsageWhichIsExplicitlyInAssetBundleLabel = new GUIContent("which is explicitly added into the Asset Bundle");
		...
		/// <summary>
		/// "is explicitly added into the Asset Bundle"
		/// </summary>
		static readonly GUIContent AssetUsageIsExplicitlyInAssetBundleLabel =
			new GUIContent("is explicitly added into the Asset Bundle");
		...
		string GetPrettyAssetPath(string assetPath, bool showAssetPath, bool selected)
		{
			...
			{
				var path = CustomBuildReport.Util.GetAssetPath(assetPath);
				var filename = CustomBuildReport.Util.GetAssetFilename(assetPath);
				...
				else
				{
					entry.AssetPathSelected = string.Format("<color=#{0}>{1}</color><color=white><b>{2}</b></color>",
						CustomBuildReport.Window.Screen.AssetList.GetPathColor(true),
						path, filename);

					entry.AssetPath = string.Format("<color=#{0}>{1}</color><b>{2}</b>",
						CustomBuildReport.Window.Screen.AssetList.GetPathColor(false),
						path, filename);
				...
				if (selected)
				{
					if (string.IsNullOrEmpty(entry.AssetPathSelected) || entry.NumberForPathSelected != number)
					{
						entry.AssetPathSelected = string.Format(" {0}. <color=#{1}>{2}</color><b>{3}</b>",
							(number + 1).ToString(),
							CustomBuildReport.Window.Screen.AssetList.GetPathColor(true),
							CustomBuildReport.Util.GetAssetPath(assetPath),
							CustomBuildReport.Util.GetAssetFilename(assetPath));
			...
			else
			{
				if (string.IsNullOrEmpty(entry.AssetName) || entry.NumberForName != number)
				{
					entry.AssetName = string.Format(" {0}. <b>{1}</b>",
						(number + 1).ToString(), CustomBuildReport.Util.GetAssetFilename(assetPath));
		...
		/// <summary>
		/// Filename and icon of the selected asset.
		/// </summary>
		readonly GUIContent _selectedAsset = new GUIContent();

		/// <summary>
		/// Path to the selected asset, starting from "Assets/"
		/// </summary>
		...
		public void DrawAssetUsage(Rect position, CustomBuildReport.AssetList listToDisplay,
			BuildInfo buildReportToDisplay, AssetDependencies assetDependencies)
		{
			...
			if (selectedHasNoAssetDependencies)
			{
				...
				//
				// If this Build Report is for Asset Bundles, then when asset has no Asset Dependencies, that means
				// it was explicitly added into the Asset Bundle.
				if (_selectedIsAResourcesAsset || buildReportToDisplay.HasAssetBundles)
				{
					     ...
					     Event.current.mousePosition.y < position.height))
					{
						...
						if (assetUsageAncestryEntryRect.Contains(Event.current.mousePosition))
						{
							...
							BRT_BuildReportWindow.UpdateHoveredAsset(_selectedAssetPath,
							...
							if (CustomBuildReport.Options.ShowTooltipThumbnail &&
							    BRT_BuildReportWindow.GetAssetPreview(_selectedAssetPath) != null)
							{
					...
					var label = buildReportToDisplay.HasAssetBundles
						? AssetUsageIsExplicitlyInAssetBundleLabel
						: AssetUsageIsAResourcesAssetLabel;
					GUILayout.Label(label, labelSingleLineStyle, BRT_BuildReportWindow.LayoutNone);
			...
			if (_showAssetUsagesList)
			{
				switch (_selectedAssetUsageDisplayIdx)
				{
					...
					case ASSET_USAGE_DISPLAY_END:
					{
						...
						{
							CustomBuildReport.AssetDependencies.PopulateAssetEndUsers(selectedAsset.Name, assetDependencies);
						}
			...
			if (_showAssetUsagesList)
			{
				...
				switch (_selectedAssetUsageDisplayIdx)
				{
					...
					case ASSET_USAGE_DISPLAY_END:
					{
						...
						if (endUsersList.Count == 0)
						{
							CustomBuildReport.AssetDependencies.PopulateAssetEndUsers(selectedAsset.Name,
			...
			out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
		{
			...
			for (; directUserN < directUserLen; ++directUserN)
			{
				...
				_assetUsageEntryLabel.text =
					GetPrettyAssetPath(assetPath, CustomBuildReport.Options.ShowColumnAssetPath, _selectedAssetUserIdx == directUserN);
				_assetUsageEntryLabel.image = CustomBuildReport.Window.Utility.GetIcon(assetPath);
			...
			out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
		{
			...
			for (; endUserN < endUserLen; ++endUserN)
			{
				...
				_assetUsageEntryLabel.text =
					GetPrettyAssetPath(assetPath, CustomBuildReport.Options.ShowColumnAssetPath, _selectedAssetUserIdx == endUserN);
			...
			out int newEntryHoveredIdx, out string newEntryHoveredAssetPath, out Rect newEntryHoveredRect)
		{
			...
			for (; userFlattenedN < userFlattenedLen; ++userFlattenedN)
			{
				...
				if (usersFlattened[userFlattenedN].IndentLevel > 1)
				{
					...
					int idxOfPrevious;
					if (CustomBuildReport.AssetDependencyGenerator.IsFileTypeBeforeAnother(usersFlattened, userFlattenedN,
				...
				_assetUsageEntryLabel.text = GetPrettyAssetPath(usersFlattened[userFlattenedN].AssetPath, CustomBuildReport.Options.ShowColumnAssetPath,
			...
			AssetDependencies assetDependencies)
		{
			...
			else if (_selectedAssetUsageDisplayIdx == ASSET_USAGE_DISPLAY_DIRECT)
			{
				...
				_assetUsageEntryLabel.text = CustomBuildReport.Util.GetAssetFilename(_assetUserCrumbs[0].AssetPath);
				...
				{
					_assetUsageEntryLabel.text = CustomBuildReport.Util.GetAssetFilename(_assetUserCrumbs[crumbN].AssetPath);
			...
			AssetDependencies assetDependencies)
		{
			...
			for (int n = 0, len = _assetUsageAncestry.Count; n < len; ++n)
			{
				...
				if (CustomBuildReport.Options.IsAssetUsageLabelTypeOnVerbose)
				{
					if (n == 0)
					{
					...
					else if (n == len - 1)
					{
						...
						{
							widthToAdd = assetLabelInBetweenStyle.CalcSize(AssetUsageWhichIsExplicitlyInAssetBundleLabel).x +
							             assetLabelInBetweenStyle.margin.horizontal;
						}
					}
				}
				else if (CustomBuildReport.Options.IsAssetUsageLabelTypeOnStandard)
				{
					if (n < len - 1)
					{
				...
				else if (CustomBuildReport.Options.IsAssetUsageLabelTypeOnMinimal)
				{
					if (n < len - 1)
					{
						widthToAdd = assetUsageArrow.width + assetUsageArrowStyle.padding.horizontal;
				...
				if (CustomBuildReport.Options.IsAssetUsageLabelTypeOnVerbose)
				{
					...
					else if (n == len - 1)
					{
						...
						{
							GUILayout.Label(AssetUsageWhichIsExplicitlyInAssetBundleLabel,
								assetLabelInBetweenStyle, BRT_BuildReportWindow.LayoutNone);
						}
					}
				}
		...
		void SetAssetUsageHistoryToFirstEndUser(string rootAsset, AssetDependencies assetDependencies)
		{
			...
			// ----------------
			// find the first deepest asset user

			int deepestLevel = 0;
			int idxOfDeepest = -1;
			for (int n = 0, len = usersFlattened.Count; n < len; ++n)
			{
				if (deepestLevel < usersFlattened[n].IndentLevel)
				{
					deepestLevel = usersFlattened[n].IndentLevel;
					idxOfDeepest = n;
				}
			}
			...
			if (idxOfDeepest != -1)
			{
				_selectedAssetUserIdx = idxOfDeepest;
				SetAssetUsageAncestry(_assetUsageAncestry, usersFlattened, idxOfDeepest, rootAsset);
				_assetUsageAncestryHasFbxUsingDefaultMaterialUsedInScene = false;
				return;
			}
			...
			int idxChosenFromSource, string rootAsset)
		{
			...
			lastEntry.Label =
				new GUIContent(
					...
					CustomBuildReport.Window.Utility.GetIcon(lastEntry.AssetPath));
			...
			for (int traceN = idxChosenFromSource - 1; traceN >= 0; --traceN)
			{
				...
				newEntry.Label =
					new GUIContent(
						...
						CustomBuildReport.Window.Utility.GetIcon(newEntry.AssetPath));
			...
			firstEntry.Label =
				new GUIContent(
					...
					CustomBuildReport.Window.Utility.GetIcon(firstEntry.AssetPath));
