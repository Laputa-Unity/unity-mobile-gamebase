	...
	public partial class AssetList
	{
		bool ShouldShowDeleteButtons(BuildInfo buildReportToDisplay)
		{
			return
				...
				(buildReportToDisplay.HasUsedAssets && CustomBuildReport.Options.AllowDeletingOfUsedAssets);
		}


		void InitiateDeleteSelectedUsed(BuildInfo buildReportToDisplay, CustomBuildReport.AssetBundleSession assetBundleSession)
		{
			CustomBuildReport.AssetList listToDeleteFrom = GetAssetListToDisplay(buildReportToDisplay, assetBundleSession);

			InitiateDeleteSelectedInAssetList(buildReportToDisplay, listToDeleteFrom);
		}

		void InitiateDeleteSelectedInAssetList(BuildInfo buildReportToDisplay, CustomBuildReport.AssetList listToDeleteFrom)
		{
			...
			CustomBuildReport.SizePart[] all = listToDeleteFrom.All;
			...
			{
				CustomBuildReport.SizePart b = all[n];
				bool isThisFileToBeDeleted = listToDeleteFrom.InSumSelection(b);

				if (isThisFileToBeDeleted)
				{
					if (CustomBuildReport.Util.IsFileInBuildReportFolder(b.Name) &&
					    !CustomBuildReport.Util.IsUselessFile(b.Name))
					{
						//Debug.Log("CBR file? " + b.Name);
						--numOfFilesToDelete;
						++brtFilesSelectedForDelete;
					}
					else if (CustomBuildReport.Util.HaveToUseSystemForDelete(b.Name))
					{
			...
			// add warning about CBR files that are skipped
			if (brtFilesSelectedForDelete > 0)
			{
			...
			if (systemDeletionFileCount > 0)
			{
				if (deletingSystemFilesOnly)
				{
					message += "\n\nThe deleted file" + plural + " will not be recoverable from the " +
					           CustomBuildReport.Util.NameOfOSTrashFolder + ", unless you have your own backup.";
				}
				else
				{
				...
				message +=
					...
					CustomBuildReport.Util.NameOfOSTrashFolder +
			...
			List<CustomBuildReport.SizePart> allList = new List<CustomBuildReport.SizePart>(all);
			List<CustomBuildReport.SizePart> toRemove = new List<CustomBuildReport.SizePart>(all.Length / 4);

			// finally, delete the files
			int deletedCount = 0;
			for (int n = 0, len = allList.Count; n < len; ++n)
			{
				CustomBuildReport.SizePart b = allList[n];


				bool okToDelete = CustomBuildReport.Util.IsUselessFile(b.Name) ||
				                  !CustomBuildReport.Util.IsFileInBuildReportFolder(b.Name);

				if (listToDeleteFrom.InSumSelection(b) && okToDelete)
				{
					// delete this

					if (CustomBuildReport.Util.ShowFileDeleteProgress(deletedCount, numOfFilesToDelete, b.Name,
						deleteIsRecoverable))
					{
						return;
					}

					CustomBuildReport.Util.DeleteSizePartFile(b);
					toRemove.Add(b);
					++deletedCount;
				}
			}
			...
			CustomBuildReport.SizePart[] allWithRemoved = allList.ToArray();

			// recreate per category list (maybe just remove from existing per category lists instead?)
			CustomBuildReport.SizePart[][] perCategoryOfList =
				CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(allWithRemoved,
					buildReportToDisplay.FileFilters);

			listToDeleteFrom.Reinit(allWithRemoved, perCategoryOfList,
				IsShowingUsedAssets
					? CustomBuildReport.Options.NumberOfTopLargestUsedAssetsToShow
					: CustomBuildReport.Options.NumberOfTopLargestUnusedAssetsToShow);
			...
			{
				finalMessage += " They can be recovered from your " + CustomBuildReport.Util.NameOfOSTrashFolder + ".";
			}
		...
		{
			CustomBuildReport.AssetList list = buildReportToDisplay.UnusedAssets;
			CustomBuildReport.SizePart[] all = list.All;
			...
			for (int n = 0, len = all.Length; n < len; ++n)
			{
				...
				bool okToDelete = CustomBuildReport.Util.IsFileOkForDeleteAllOperation(b.Name);
			...
			if (!EditorUtility.DisplayDialog("Delete?",
				    string.Format(
					    ...
					    CustomBuildReport.Util.NameOfOSTrashFolder), "Yes", "No"))
			{
				return;
			}

			List<CustomBuildReport.SizePart> newAll = new List<CustomBuildReport.SizePart>();
			...
			for (int n = 0, len = all.Length; n < len; ++n)
			{
				...
				{
					// delete this
					if (CustomBuildReport.Util.ShowFileDeleteProgress(deletedCount, filesToDeleteCount, b.Name, true))
					{
						return;
					}
			...
			CustomBuildReport.SizePart[] newAllArr = newAll.ToArray();

			CustomBuildReport.SizePart[][] perCategoryUnused =
				CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(newAllArr, buildReportToDisplay.FileFilters);
			...
			string finalMessage = string.Format(
				...
				filesToDeleteCount.ToString(), plural, CustomBuildReport.Util.NameOfOSTrashFolder);
			Debug.LogWarning(finalMessage);
