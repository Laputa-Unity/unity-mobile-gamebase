	...
	StringBuilder _debugText;
		...
		void DrawOverlay(BuildReportTool.BuildInfo buildReportToDisplay, bool isOverlay = true)
		{
			...
			if (_showSearchOptions)
			{
				...
				_columnLabel.text = "Custom";
				rect.size = radioRightStyle.CalcSize(_columnLabel);
				var pressedCustom = GUI.Toggle(rect, BuildReportTool.Options.SearchTypeIsCustom, _columnLabel, radioRightStyle);
				if (rect.Contains(Event.current.mousePosition))
				{
					_overridenTextureDataTooltipText = "<b><color=white>Custom Search</color></b>\n\nApproximate string matching.";
				}
				searchTypeHeight = Mathf.Max(searchTypeHeight, rect.height);

				if (pressedCustom && !BuildReportTool.Options.SearchTypeIsCustom)
				{
					BuildReportTool.Options.SearchType = SearchType.Custom;
					UpdateSearchNow(buildReportToDisplay);
				}
				...
				var usingCustom = BuildReportTool.Options.SearchTypeIsCustom;
				GUI.enabled = prevEnabled && !usingCustom;
				...
				if (usingCustom && rect.Contains(Event.current.mousePosition))
				{
					_overridenTextureDataTooltipText = "Not applicable to Custom Search. Custom Search is always Case Insensitive.";
				}
				rect.y += rect.height + TOGGLE_SPACING;
