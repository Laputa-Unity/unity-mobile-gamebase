...
namespace CustomBuildReport.Window.Screen
{
	public class Overview : BaseScreen
	{
			...
			out bool requestRepaint)
		{
			...
			var titleStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.MAIN_TITLE_STYLE_NAME);
			if (titleStyle == null)
			{
			...
			var bigValueStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.INFO_SUBTITLE_STYLE_NAME);
			if (bigValueStyle == null)
			{
				bigValueStyle = GUI.skin.label;
			}

			var bigNumberStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.BIG_NUMBER_STYLE_NAME);
			if (bigNumberStyle == null)
			{
				bigNumberStyle = GUI.skin.label;
			}

			var smallLabelStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.SETTING_NAME_STYLE_NAME);
			if (smallLabelStyle == null)
			{
				smallLabelStyle = GUI.skin.label;
			}

			var smallValueStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.SETTING_VALUE_STYLE_NAME);
			if (smallValueStyle == null)
			{
				smallValueStyle = GUI.skin.label;
			}

			var helpDescriptionStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.TINY_HELP_STYLE_NAME);
			if (helpDescriptionStyle == null)
			{
				helpDescriptionStyle = GUI.skin.label;
			}

			var infoDescriptionStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.INFO_TEXT_STYLE_NAME);
			    ...
			    !string.IsNullOrEmpty(buildReportToDisplay.BuildFilePath))
			{
				...
				GUILayout.Label(CustomBuildReport.Util.GetBuildSizePathDescription(buildReportToDisplay), helpDescriptionStyle);
			...
			if (_showTopUsed)
			{
				...
				CustomBuildReport.SizePart[] topLargestToDisplay;
				if (buildReportToDisplay.HasAssetBundles)
				{
			...
			var showThumbnailNow = CustomBuildReport.Options.ShowTooltipThumbnail &&
		...
		void DrawAssetList(CustomBuildReport.SizePart[] assetsToShow, bool usedAssets, BuildInfo buildReportToDisplay,
			AssetDependencies assetDependencies)
		{
			...
			var listIconNormalStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.LIST_ICON_STYLE_NAME);
			if (listIconNormalStyle == null)
			{
				listIconNormalStyle = GUI.skin.label;
			}

			var listIconAltStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.LIST_ICON_ALT_STYLE_NAME);
			...
			CustomBuildReport.SizePart newEntryHovered = null;
			...
			{
				CustomBuildReport.SizePart b = assetsToShow[n];
				...
				string prettyName = string.Format(" {0}. {1}", (n + 1).ToString(), CustomBuildReport.Util.GetAssetFilename(b.Name));
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				if (newEntryHoveredIdx != -1)
				{
					...
					if (CustomBuildReport.Options.ShowTooltipThumbnail &&
					    (CustomBuildReport.Options.ShowThumbnailOnHoverLabelToo || hoveringOverIcon) &&
			...
			var useRawSize = (usedAssets && !CustomBuildReport.Options.ShowImportedSizeForUsedAssets) || !usedAssets;
		...
		void DrawAssetBundleList(CustomBuildReport.BundleEntry[] bundles)
		{
		...
		void DrawScenesInBuild(CustomBuildReport.BuildInfo.SceneInBuild[] scenesInBuild)
		{
			...
			for (int n = 0; n < scenesInBuild.Length; ++n)
			{
				...
				{
					var pathName = CustomBuildReport.Util.GetAssetPath(scenesInBuild[n].Path);
