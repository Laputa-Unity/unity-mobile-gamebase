...
namespace CustomBuildReport.Window.Screen
{
	public class BuildSettings : BaseScreen
	{
		...
		public override void RefreshData(BuildInfo buildReport, AssetDependencies assetDependencies, TextureData textureData, MeshData meshData, PrefabData prefabData, UnityBuildReport unityBuildReport, CustomBuildReport.AssetBundleSession assetBundleSession)
		{
		...
		void DrawSetting(string name, string val, bool showEvenIfEmpty = true)
		{
			...
			var nameStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.SETTING_NAME_STYLE_NAME);
			if (nameStyle == null)
			{
				nameStyle = GUI.skin.label;
			}

			var valueStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.SETTING_VALUE_NO_WRAP_STYLE_NAME);
			if (valueStyle == null)
			{
		...
		{
			var titleStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.INFO_TITLE_STYLE_NAME);
			if (titleStyle == null)
			{
		...
		void DrawPackageSettings(BuildInfo buildReportToDisplay, UnityBuildSettings settings)
		{
			...
			var valueStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.SETTING_VALUE_STYLE_NAME);
			if (valueStyle == null)
			{
				valueStyle = GUI.skin.label;
			}
			...
			void DrawPackageEntry(string packageName, string displayName, string versionUsed, string location, string localPath)
			{
				...
				if (!string.IsNullOrEmpty(location) && location != CustomBuildReport.UnityBuildSettingsUtility.DEFAULT_REGISTRY_URL)
				{
		...
		void DrawPackagePingButton(string packageName, string localPath)
		{
			if (!string.IsNullOrEmpty(localPath))
			{
				...
				{
					CustomBuildReport.Util.OpenInFileBrowser(localPath);
				}
			}
		}
			...
			out bool requestRepaint)
		{
			...
			var fileFilterPopupStyle = GUI.skin.FindStyle(CustomBuildReport.Window.Settings.FILE_FILTER_POPUP_STYLE_NAME);
			...
			CustomBuildReport.Options.ShowProjectSettingsInMultipleColumns = GUILayout.Toggle(CustomBuildReport.Options.ShowProjectSettingsInMultipleColumns, "Multiple Columns");
			GUILayout.Space(30);
			...
			var showMultiColumn = CustomBuildReport.Options.ShowProjectSettingsInMultipleColumns;
