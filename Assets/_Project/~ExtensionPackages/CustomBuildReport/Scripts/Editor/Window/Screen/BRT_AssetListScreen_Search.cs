...
namespace BuildReportTool.Window.Screen
{
	public partial class AssetList
	{
		...
		readonly CustomStringComparisonOptions[] _searchOptions =
		{
			CustomStringComparisonOptions.UseOverlapCoefficient,
			CustomStringComparisonOptions.UseLongestCommonSubsequence,
			CustomStringComparisonOptions.UseLongestCommonSubstring
		};
		...
		void Search(string searchText, SearchType searchType, bool searchFilenameOnly, bool caseSensitive, BuildInfo buildReportToDisplay)
		{
			...
			if (searchResults.Count > 0)
			{
				searchResults.Sort((a, b) =>
					GetCustomEqualityScore(searchText, a.Name).CompareTo(GetCustomEqualityScore(searchText, b.Name)));
			}
		...
		void SortBySearchRank(BuildReportTool.SizePart[] assetList, string searchText)
		{
			...
			System.Array.Sort(assetList, (entry1, entry2) =>
				GetCustomEqualityScore(searchText, entry1.Name)
					.CompareTo(GetCustomEqualityScore(searchText, entry2.Name)));
		}
		...
		bool IsANearStringMatch(string source, string target)
		{
			...
			const CustomStringComparisonTolerance TOLERANCE = CustomStringComparisonTolerance.Strong;

			// Get a boolean determination of approximate equality
			return source.ApproximatelyEquals(target, TOLERANCE, _searchOptions);
		}

		double GetCustomEqualityScore(string source, string target)
		{
			if (string.IsNullOrEmpty(target))
			{
				return 0;
			...
			return source.GetCustomEqualityScore(target, _searchOptions);
		}
	}
}