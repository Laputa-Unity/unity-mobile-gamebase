...
namespace CustomBuildReport.Window.Screen
{
	public class Help : BaseScreen
	{
			...
			TextureData textureData, MeshData meshData, PrefabData prefabData, UnityBuildReport unityBuildReport, CustomBuildReport.AssetBundleSession assetBundleSession)
		{
			...
			string readmeContents = CustomBuildReport.Util.GetPackageFileContents(README_FILENAME);

			const string CHANGELOG_FILENAME = "VERSION.txt";
			string changelogContents = CustomBuildReport.Util.GetPackageFileContents(CHANGELOG_FILENAME);
