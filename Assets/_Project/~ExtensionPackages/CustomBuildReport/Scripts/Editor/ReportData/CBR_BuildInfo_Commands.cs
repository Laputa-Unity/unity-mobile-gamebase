...
namespace CustomBuildReport
{
	public partial class BuildInfo
	{
		...
		public string GetTimeReadable()
		{
			...
			return TimeGot.ToString(CustomBuildReport.ReportGenerator.TIME_OF_BUILD_FORMAT);
		}
		...
		{
			return CustomBuildReport.Util.GetBuildInfoDefaultFilename(ProjectName, BuildType, BuildTimeGot);
		}

		public string GetAccompanyingAssetDependenciesFilename()
		{
			return CustomBuildReport.Util.GetAssetDependenciesDefaultFilename(ProjectName, BuildType, BuildTimeGot);
		}
		...
		public void RecategorizeAssetLists()
		{
			...
			{
				fileFiltersToUse = CustomBuildReport.FiltersUsed.GetProperFileFilterGroupToUse();
			...
			if (UsedAssets != null)
			{
				UsedAssets.AssignPerCategoryList(
					CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(UsedAssets.All, fileFiltersToUse));

				UsedAssets.RefreshFilterLabels(fileFiltersToUse);

				UsedAssets.ResortDefault(CustomBuildReport.Options.NumberOfTopLargestUsedAssetsToShow);
			}

			if (UnusedAssets != null)
			{
				UnusedAssets.AssignPerCategoryList(
					CustomBuildReport.ReportGenerator.SegregateAssetSizesPerCategory(UnusedAssets.All, fileFiltersToUse));

				UnusedAssets.RefreshFilterLabels(fileFiltersToUse);

				UnusedAssets.ResortDefault(CustomBuildReport.Options.NumberOfTopLargestUnusedAssetsToShow);
			}
		}
		...
		void CalculateUsedAssetsDerivedSizes()
		{
			if (UsedAssets != null)
			{
				...
				{
					UsedAssets.All[n].DerivedSize = CustomBuildReport.Util.GetApproxSizeFromString(UsedAssets.All[n].Size);
				}
			}
		}
		...
		public void SortSizes()
		{
			...
			System.Array.Sort(BuildSizes, delegate(CustomBuildReport.SizePart b1, CustomBuildReport.SizePart b2)
			{
		...
		public void FixReport()
		{
			...
			if (!DldUtil.UnityVersion.IsUnityVersionAtLeast(5, 2, 1))
			{
				...
				{
					if (CustomBuildReport.Util.IsFileInAPath(UsedAssets.All[n].Name, "/Resources/"))
					{
						...
						UsedAssets.All[n].ImportedSize =
							CustomBuildReport.Util.GetBytesReadable(UsedAssets.All[n].ImportedSizeBytes);
				...
				var resourcesTextureSizeSum =
					GetSizeSumForUsedAssets("/Resources/", CustomBuildReport.Util.IsTextureFile);
				AddToSize("Textures", resourcesTextureSizeSum);

				var resourcesMeshSizeSum = GetSizeSumForUsedAssets("/Resources/", CustomBuildReport.Util.IsMeshFile);
				AddToSize("Meshes", resourcesMeshSizeSum);

				var resourcesSoundsSizeSum = GetSizeSumForUsedAssets("/Resources/", CustomBuildReport.Util.IsSoundFile);
				AddToSize("Sounds", resourcesSoundsSizeSum);

				var resourcesAnimationsSizeSum =
					GetSizeSumForUsedAssets("/Resources/", CustomBuildReport.Util.IsAnimationFile);
		...
		long GetSizeSumForUsedAssets(string assetFolderName, System.Func<string, bool> fileTypePredicate)
		{
			...
			return UsedAssets.All.Where(part =>
				                 CustomBuildReport.Util.IsFileInAPath(part.Name, assetFolderName) &&
				                 fileTypePredicate(part.Name))
			                 .Sum(part => BRT_LibCacheUtil.GetImportedFileSize(part.Name));
		}

		static void AddToSize(CustomBuildReport.SizePart buildSize, long sizeToAdd)
		{
			if (buildSize != null)
			{
				buildSize.DerivedSize += sizeToAdd;
				buildSize.Size = CustomBuildReport.Util.GetBytesReadable(buildSize.DerivedSize);
			}
		}
		...
		void AddToSize(string buildSizeName, long sizeToAdd)
		{
			...
			CustomBuildReport.SizePart buildSize = BuildSizes.FirstOrDefault(part => part.Name == buildSizeName);
		...
		void AddToTotalSize(long sizeToAdd)
		{
			...
			CustomBuildReport.SizePart buildSize = BuildSizes.FirstOrDefault(part => part.IsTotal);
		...
		void ChangeTotalSize(double newSize)
		{
			...
			CustomBuildReport.SizePart totalSize = BuildSizes.FirstOrDefault(part => part.IsTotal);
			...
			if (totalSize != null)
			{
				...
				totalSize.Size = CustomBuildReport.Util.GetBytesReadable(totalSize.DerivedSize);
