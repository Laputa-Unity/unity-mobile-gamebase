...
namespace CustomBuildReport
{
	...
	public class AssetDependencies : CustomBuildReport.IDataFile
	{
		/// <summary>
		...
		{
			return CustomBuildReport.Util.GetAssetDependenciesDefaultFilename(ProjectName, BuildType, TimeGot);
		}
			...
			for (int n = 0, len = usersFlattened.Count; n < len; ++n)
			{
				    ...
				    isAssembly)
				{
					...
					Texture icon = isAssembly
						? CustomBuildReport.Window.Utility.AssemblyIcon
