...
namespace CustomBuildReport
{
	...
	public class AssetList
	{
		...
		[SerializeField]
		CustomBuildReport.SizePart[] _all;

		int[] _viewOffsets;

		[SerializeField]
		CustomBuildReport.SizePart[][] _perCategory;
		...
		public CustomBuildReport.SizePart[] All
		{
			get { return _all; }
			set { _all = value; }
		}

		public CustomBuildReport.SizePart[][] PerCategory
		{
		...
		CustomBuildReport.SizePart[] _topLargest;
		...
		{
			List<CustomBuildReport.SizePart> topLargestList = new List<CustomBuildReport.SizePart>();
		...
		CustomBuildReport.TextureData.DataId _lastTextureSortType = CustomBuildReport.TextureData.DataId.None;
		CustomBuildReport.MeshData.DataId _lastMeshSortType = CustomBuildReport.MeshData.DataId.None;
		CustomBuildReport.PrefabData.DataId _lastPrefabSortType = CustomBuildReport.PrefabData.DataId.None;
		...
		public void Resort(CustomBuildReport.SizePart[] assetList)
		{
			if (_lastSortType != SortType.None &&
			    _lastTextureSortType == CustomBuildReport.TextureData.DataId.None &&
			    _lastMeshSortType == CustomBuildReport.MeshData.DataId.None &&
		...
		public void Sort(CustomBuildReport.TextureData textureData, CustomBuildReport.TextureData.DataId sortType, SortOrder sortOrder, CustomBuildReport.FileFilterGroup fileFilters)
		{
		...
		public void Sort(CustomBuildReport.MeshData meshData, CustomBuildReport.MeshData.DataId sortType, SortOrder sortOrder, CustomBuildReport.FileFilterGroup fileFilters)
		{
			_lastTextureSortType = CustomBuildReport.TextureData.DataId.None;
		...
		public void Sort(CustomBuildReport.PrefabData prefabData, CustomBuildReport.PrefabData.DataId sortType, SortOrder sortOrder, CustomBuildReport.FileFilterGroup fileFilters)
		{
			_lastTextureSortType = CustomBuildReport.TextureData.DataId.None;
		...
		public void Sort(SortType sortType, SortOrder sortOrder, CustomBuildReport.FileFilterGroup fileFilters)
		{
			_lastTextureSortType = CustomBuildReport.TextureData.DataId.None;
			_lastMeshSortType = CustomBuildReport.MeshData.DataId.None;
		...
		public void SortIfNeeded(CustomBuildReport.FileFilterGroup fileFilters)
		{
			if (_lastSortType != SortType.None &&
			    _lastSortOrder != SortOrder.None &&
		...
		public List<CustomBuildReport.SizePart> GetAllAsList()
		{
			return _all.ToList();
		}
		...
		public CustomBuildReport.SizePart[] GetListToDisplay(FileFilterGroup fileFilters)
		{
			CustomBuildReport.SizePart[] ret = null;
			if (fileFilters.SelectedFilterIdx == -1)
			{
				ret = All;
			}
		...
		public void UnescapeAssetNames()
		{
			if (_all != null)
			{
				for (int n = 0, len = _all.Length; n < len; ++n)
				{
					_all[n].Name = CustomBuildReport.Util.MyHtmlDecode(_all[n].Name);
				}
			}
			...
			if (_perCategory != null)
			{
				for (int catIdx = 0, catLen = _perCategory.Length; catIdx < catLen; ++catIdx)
				{
					...
					{
						_perCategory[catIdx][n].Name = CustomBuildReport.Util.MyHtmlDecode(_perCategory[catIdx][n].Name);
					}
				}
			}
		}
		...
		public void PopulateRawSizes()
		{
			...
			for (int n = 0, len = _all.Length; n < len; ++n)
			{
				...
				_all[n].Size = CustomBuildReport.Util.GetBytesReadable(importedSize);
			}*/
		}
		...
		public void PopulateImportedSizes()
		{
			for (int n = 0, len = _all.Length; n < len; ++n)
			{
				/*if (CustomBuildReport.Util.IsFileAUnityAsset(_all[n].Name))
				{
				...
				else*/
				{
					...
					_all[n].ImportedSize = CustomBuildReport.Util.GetBytesReadable(importedSize);
				}
			}
		}
		...
		public void PopulateSizeInAssetsFolder()
		{
			...
			var projectPath = CustomBuildReport.Util.GetProjectPath(Application.dataPath);
			for (int n = 0, len = _all.Length; n < len; ++n)
			{
				string assetImportedPath = projectPath + CustomBuildReport.Util.MyHtmlDecode(_all[n].Name);

				var size = CustomBuildReport.Util.GetFileSizeInBytes(assetImportedPath);
				_all[n].SizeInAssetsFolderBytes = size;
				_all[n].SizeInAssetsFolder = CustomBuildReport.Util.GetBytesReadable(size);
			}
		}
		...
		public void Init(CustomBuildReport.SizePart[] all, CustomBuildReport.SizePart[][] perCategory, int numberOfTop,
		...
		public void Reinit(CustomBuildReport.SizePart[] all, CustomBuildReport.SizePart[][] perCategory, int numberOfTop)
		{
			All = all;
			PostSetListAll(numberOfTop);
			_perCategory = perCategory;
		...
		public void AssignPerCategoryList(CustomBuildReport.SizePart[][] perCategory)
		{
			_perCategory = perCategory;
		...
		[SerializeField]
		Dictionary<string, CustomBuildReport.SizePart> _selectedForSum = new Dictionary<string, CustomBuildReport.SizePart>();

		CustomBuildReport.SizePart _lastSelected;


		// Sum Selection: Queries
		// --------------------------------------------------------------------

		public bool InSumSelection(CustomBuildReport.SizePart b)
		{
		...
		{
			return CustomBuildReport.Util.GetBytesReadable(GetSizeOfSumSelection());
		}
		...
		public void ToggleSumSelection(CustomBuildReport.SizePart b)
		{
			if (InSumSelection(b))
			{
		...
		public void RemoveFromSumSelection(CustomBuildReport.SizePart b)
		{
			_selectedForSum.Remove(b.Name);
		}

		public void AddToSumSelection(CustomBuildReport.SizePart b)
		{
		...
		{
			CustomBuildReport.SizePart[] listForSelection = GetListToDisplay(fileFilters);
