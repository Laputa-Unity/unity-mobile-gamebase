...
namespace CustomBuildReport
{
	public partial class ReportGenerator
	{
		...
		{
			CustomBuildReport.Util.SaveBuildTime();
		}
		...
		/// CustomBuildReport will just use the default location of the Editor.log file.</param>
		///
		/// <param name="customSavePath">Path to folder where the Build Report XML file will be saved.
		/// Leave this null so that CustomBuildReport will just use the path specified in the saved options.</param>
		///
		...
		/// so that CustomBuildReport will just use UnityEditor.EditorBuildSettings.scenes instead.
		...
		/// CustomBuildReport will just use Unity's <see cref="UnityEditor.EditorUserBuildSettings.GetBuildLocation"/>
		...
		/// default path, specify it here. Otherwise, leave this null so that CustomBuildReport
		...
		public static string CreateReport(string[] scenes, string buildLocation, BuildTarget buildTarget,
			string customEditorLogPath = null, string customSavePath = null)
		{
			CustomBuildReport.Util.BuildTargetOfLastBuild = buildTarget;

			var editorLogPathToUse = !string.IsNullOrEmpty(customEditorLogPath)
				                         ? customEditorLogPath
				                         : CustomBuildReport.Util.UsedEditorLogPath;
			...
			if (CustomBuildReport.Util.HasBuildTime())
			{
			...
			if (CustomBuildReport.Options.CalculateAssetDependencies)
			{
				...
				{
					_lastKnownAssetDependencies = new CustomBuildReport.AssetDependencies();
				}
			...
			if (CustomBuildReport.Options.CollectTextureImportSettings)
			{
				...
				{
					_lastKnownTextureData = new CustomBuildReport.TextureData();
				}
			...
			if (CustomBuildReport.Options.CollectMeshData)
			{
				...
				{
					_lastKnownMeshData = new CustomBuildReport.MeshData();
				}
			...
			if (CustomBuildReport.Options.CollectPrefabData)
			{
				...
				{
					_lastKnownPrefabData = new CustomBuildReport.PrefabData();
				}
