...
namespace CustomBuildReport
{
	...
	{
		public static void Create(TextureData data, CustomBuildReport.BuildInfo buildInfo, bool createForUnusedAssetsToo, bool debugLog = false)
		{
			if (buildInfo == null)
			{
			...
			CustomBuildReport.BuildPlatform buildPlatform = CustomBuildReport.ReportGenerator.GetBuildPlatformFromString(buildInfo.BuildType, buildInfo.BuildTargetUsed);
		...
		static void AppendTextureData(TextureData data, CustomBuildReport.BuildPlatform buildPlatform, IList<SizePart> assets, bool overwriteExistingEntries, bool debugLog = false)
		{
		...
		static TextureData.Entry CreateEntry(string assetPath, string platform, bool debugLog = false)
		{
			...
			if (importError)
			{
				result.TextureFormat = CustomBuildReport.TextureData.IMPORT_ERROR;
			}
			...
			if (overrideSettings != null && overrideSettings.overridden)
			{
				...
				if (importError)
				{
					result.OverridingTextureFormat = CustomBuildReport.TextureData.IMPORT_ERROR;
				}
			...
			if (textureImporter.npotScale == TextureImporterNPOTScale.None)
			{
				...
				{
					result.NPotScale = CustomBuildReport.TextureData.NPOT_SCALE_NONE_IS_POT;
				}
				else
				{
					result.NPotScale = CustomBuildReport.TextureData.NPOT_SCALE_NONE_NOT_POT;
				}
			}
			else
			{
