...
namespace CustomBuildReport
{
			...
			if (_usesExactFileMatching)
			{
				...
				for (int n = 0, len = _filtersList.Length; n < len; ++n)
				{
					...
					if (_filtersList[n].StartsWith("\"") && _filtersList[n].EndsWith("\""))
					{
						...
						if (_filtersList[n].Equals(fileWithQuotes, StringComparison.OrdinalIgnoreCase))
						{
							return true;
						}
					}
	...
	public class FileFilterGroup
	{
		...
		{
			CustomBuildReport.Options.FileFilterDisplay displayType = CustomBuildReport.Options.GetOptionFileFilterDisplay();
			switch (displayType)
			{
				case CustomBuildReport.Options.FileFilterDisplay.DropDown:
					return DrawFiltersAsDropDown(assetList, width);
				case CustomBuildReport.Options.FileFilterDisplay.Buttons:
