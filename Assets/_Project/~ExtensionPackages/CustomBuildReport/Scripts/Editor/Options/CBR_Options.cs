...
namespace CustomBuildReport
{
	...
	public class SavedOptions
	{
		...
		public string BuildReportFolderName = CustomBuildReport.Options.BUILD_REPORTS_DEFAULT_FOLDER_NAME;

		public string BuildReportCustomOutputPath = CustomBuildReport.Util.GetUserHomeFolder();
	...
	public static class Options
	{
		...
		// =======================================================
		//
		static CustomBuildReport.SavedOptions _savedOptions;
		...
		{
			get { return string.Format("{0}ProjectSettings/{1}", CustomBuildReport.Util.GetProjectPath(Application.dataPath), SAVED_OPTIONS_FILENAME); }
		}
		...
		static void InitializeOptionsIfNeeded()
		{
			...
			if (string.IsNullOrEmpty(_foundPathForSavedOptions))
			{
				...
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInBuildReportFolder);
				...
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInPluginsBuildReport);
				...
				if (!IsBuildReportInRegularPaths)
				{
					string customBuildReportFolder =
						CustomBuildReport.Util.FindAssetFolder(Application.dataPath, BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME);
					if (!string.IsNullOrEmpty(customBuildReportFolder))
					{
						...
						{
							_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInCustomBuildReportFolder);
				...
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInTopmostAssets);
				...
				var outsideAssets = CustomBuildReport.Util.GetProjectPath(Application.dataPath);
				var optionsOutsideAssets = string.Format("{0}{1}", outsideAssets, SAVED_OPTIONS_FILENAME);
				if (File.Exists(optionsOutsideAssets))
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsOutsideAssets);
				...
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInProjectSettings);
				...
				var optionsInMyDocs = string.Format("{0}/{1}/{2}", CustomBuildReport.Util.GetUserHomeFolder(),
				...
				{
					_savedOptions = CustomBuildReport.SavedOptions.Load(optionsInMyDocs);
			...
			if (_savedOptions == null)
			{
				if (!string.IsNullOrEmpty(_foundPathForSavedOptions) && File.Exists(_foundPathForSavedOptions))
				{
					...
					// just load that one
					_savedOptions = CustomBuildReport.SavedOptions.Load(_foundPathForSavedOptions);
				}
			}
			...
			if (_savedOptions == null)
			{
				_savedOptions = new CustomBuildReport.SavedOptions();
		...
		public static string BuildReportSavePath
		{
			get
			{
				...
				{
					return string.Format("{0}/{1}", CustomBuildReport.Util.GetUserHomeFolder(), BuildReportFolderName);
				}
				else if (CustomBuildReport.Options.SaveType == CustomBuildReport.Options.SAVE_TYPE_PROJECT)
				{
					...
					return string.Format("{0}/{1}", CustomBuildReport.ReportGenerator.GetSavePathToProjectFolder(), BuildReportFolderName);
				}
				else // CustomBuildReport.Options.SAVE_TYPE_CUSTOM
				{
					...
					else
					{
						...
						return Path.GetFullPath(Path.Combine(CustomBuildReport.Util.GetProjectPath(Application.dataPath),
