...
[assembly: RegisterCustomAttributeDrawer(typeof(TableListDrawer), CustomDrawerOrder.Drawer)]

namespace CustomInspector.Drawers
{
    public class TableListDrawer : CustomAttributeDrawer<TableListAttribute>
    {
        ...
        private class TableElement : CustomListElement
        {
            ...
            public TableElement(CustomProperty property) : base(property)
            {
                _property = property;
            ...
            protected override CustomElement CreateItemElement(CustomProperty property)
            {
        ...
        {
            private readonly CustomProperty _property;
            private readonly CustomElement _cellElementContainer;
            ...
            public TableMultiColumnTreeView(CustomProperty property, CustomElement container, ReorderableList listGui)
            ...
            protected override void RowGUI(RowGUIArgs args)
            {
                ...
                for (var i = 0; i < multiColumnHeader.state.visibleColumns.Length; i++)
                {
                    ...
                    using (CustomGuiHelper.PushLabelWidth(EditorGUIUtility.labelWidth / rowElement.ChildrenCount))
                    using (CustomPropertyOverrideContext.BeginOverride(_propertyOverrideContext))
                    {
        ...
        public class TableRowElement : CustomPropertyCollectionBaseElement
        {
            public TableRowElement(CustomProperty property)
            {
                DeclareGroups(property.ValueType);

                Elements = new List<KeyValuePair<CustomElement, GUIContent>>();

                if (property.PropertyType == CustomPropertyType.Generic)
                {
                    foreach (var childProperty in property.ChildrenProperties)
                    {
                        var oldChildrenCount = ChildrenCount;

                        var props = new CustomPropertyElement.Props
                        {
                            forceInline = true,
                        };
                        ...
                        if (oldChildrenCount != ChildrenCount)
                        {
                            ...
                            Elements.Add(new KeyValuePair<CustomElement, GUIContent>(element, headerContent));
                        }
                    }
                }
                else
                {
                    var element = new CustomPropertyElement(property, new CustomPropertyElement.Props
                    {
                        forceInline = true,
                    });
            ...
            public List<KeyValuePair<CustomElement, GUIContent>> Elements { get; }
        }
        ...
        {
            public TableTreeItem(int id, CustomProperty property) : base(id, 0)
            {
                Property = property;
            }
        ...
        private class TableListPropertyOverrideContext : CustomPropertyOverrideContext
        {
            private readonly CustomProperty _grandParentProperty;
            private readonly GUIContent _noneLabel = GUIContent.none;

            public TableListPropertyOverrideContext(CustomProperty grandParentProperty)
            {
            ...
            {
                if (property.PropertyType == CustomPropertyType.Primitive &&
