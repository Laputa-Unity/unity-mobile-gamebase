...
{
    public class CustomHorizontalGroupElement : CustomPropertyCollectionBaseElement
    {
        private readonly float[] _sizes;
        private readonly float _totalFixedSize;

        public CustomHorizontalGroupElement(float[] sizes = null)
        {
        ...
        public override void OnGUI(Rect position)
        {
            ...
            for (var i = 0; i < ChildrenCount; i++)
            {
                ...
                using (CustomGuiHelper.PushLabelWidth(EditorGUIUtility.labelWidth / ChildrenCount))
                {
