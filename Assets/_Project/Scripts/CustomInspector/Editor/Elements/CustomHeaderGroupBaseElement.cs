...
{
    public abstract class CustomHeaderGroupBaseElement : CustomPropertyCollectionBaseElement
    {
        ...
        private readonly List<CustomProperty> _properties = new List<CustomProperty>();
        ...
        protected CustomHeaderGroupBaseElement(Props props = default)
        {
            _props = props;
        }
        ...
        public sealed override void OnGUI(Rect position)
        {
            ...
            if (contentHeight > 0)
            {
                CustomEditorGUI.DrawBox(contentBgRect, headerHeight > 0f
                    ? CustomEditorStyles.ContentBox
                    : CustomEditorStyles.Box);

                using (CustomGuiHelper.PushLabelWidth(EditorGUIUtility.labelWidth - InsetLeft))
                {
