...
{
    internal static class CustomManagedReferenceGui
    {
        public static void DrawTypeSelector(Rect rect, CustomProperty property)
        {
            var typeName = property.ValueType != null
                ? CustomTypeUtilities.GetTypeNiceName(property.ValueType)
                : "[None]";
        ...
        private class ReferenceTypeDropDown : AdvancedDropdown
        {
            ...
            public ReferenceTypeDropDown(CustomProperty property, AdvancedDropdownState state) : base(state)
            {
                _property = property;
            ...
            {
                var types = CustomReflectionUtilities
                    .AllNonAbstractTypes
            ...
            private class ReferenceTypeItem : AdvancedDropdownItem
            {
                public ReferenceTypeItem(Type type, Texture2D preview = null)
                    : base(type != null ? CustomTypeUtilities.GetTypeNiceName(type) : "[None]")
                {
                    Type = type;
                    icon = preview;
                }
