...
namespace Animancer.Editor
{
    ...
    public sealed class SerializableEventSequenceDrawer : PropertyDrawer
    {
        ...
        private readonly List<AnimBool>
            EventVisibility = new List<AnimBool>();

        private AnimBool GetVisibility(Context context, int index)
        {
            while (EventVisibility.Count <= index)
            {
                var visible = context.Property.isExpanded || context.SelectedEvent == index;
                EventVisibility.Add(new AnimBool(visible, Repaint));
            }

            return EventVisibility[index];
        }
